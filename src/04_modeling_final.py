...
OUT_DIR  = os.path.join(ROOT_DIR, "results", "combined_modeling_results")
os.makedirs(OUT_DIR, exist_ok=True)

CATEGORIES = {
    "keyboard": {
        "data_path":  os.path.join(DATA_DIR, "naver_keyboard_with_features.csv"),
        "fallback":   os.path.join(DATA_DIR, "naver_keyboard_merged_final.csv"),
        "valid_cat4": {"무선키보드", "유선키보드", "키패드"},
        "cat_keyword":"키보드",
        "label":      "키보드",
    },
    "chair": {
        "data_path":  os.path.join(DATA_DIR, "naver_chair_with_features.csv"),
        "fallback":   os.path.join(DATA_DIR, "naver_chair_merged_final.csv"),
...
def make_stage1_rf():
    """1단계 후보군 생성용 RandomForest. make_models()의 RF 설정과 맞춤."""
...
def eval_cls(name, model, X, y, label, cv=cv_cls):
    ...
    # 랭킹 지표도 동일한 model 설정으로 계산
    rank_m = ranking_metrics_cv(model, X, y, k=38, cv=cv)
...
def ranking_metrics_cv(model, X, y, k=38, cv=cv_cls):
    """Precision@k, Recall@k, NDCG@k — 교차검증 기반"""
...
def shap_charts(sv, X, feat_names, prefix, title_suffix, out_dir):
    # SHAP 버전에 따라 binary classifier는 [class0, class1] list를 반환할 수 있음
...
for cat_name, cfg in CATEGORIES.items():
    ...
    # OOF 방식으로 후보군 추출 — 각 샘플이 자신을 학습하지 않은 모델에서 예측받음
    oof_proba = np.zeros(len(X))
    for tr, te in cv_cls.split(X, y100):
        rf_cv = make_stage1_rf()
        rf_cv.fit(X[tr], y100[tr])
        oof_proba[te] = rf_cv.predict_proba(X[te])[:, 1]
    top100_idx = np.argsort(oof_proba)[::-1][:500]
    ...
    print(f"\n[분류] 2단계: 리뷰수 제외")
    # OOF 방식으로 후보군 추출
    oof_proba_nr = np.zeros(len(X_no_rev))
    for tr, te in cv_cls.split(X_no_rev, y100):
        rf_cv_nr = make_stage1_rf()
        rf_cv_nr.fit(X_no_rev[tr], y100[tr])
        oof_proba_nr[te] = rf_cv_nr.predict_proba(X_no_rev[te])[:, 1]
    top100_idx_nr = np.argsort(oof_proba_nr)[::-1][:500]
    ...
    for n, m in make_models(spw=neg_pos_nr).items():
        r = eval_cls(n, m, X2_nr, y2_nr, "top38(2단계,리뷰수제외)")
        r["category"] = label; cls_results.append(r)
    ...
    imp_nr = shap_charts(sv_nr, X2_nr, NO_REVIEW,
                         ...
                         f"2단계 LightGBM {label} (top38, 리뷰수 제외)", OUT_DIR)
