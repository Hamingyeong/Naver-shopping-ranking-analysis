...
def make_stage1_rf():
    """1단계 후보군 생성용 RF. make_models()의 RandomForest 설정과 동일하게 맞춤."""
...
def eval_cls(name, model, X, y, label, cv=cv_cls):
    ...
    # 랭킹 지표도 위 cross_validate와 같은 모델 설정으로 계산
...
def ranking_metrics_cv(model, X, y, k=38, cv=cv_cls):
    """Precision@k, Recall@k, NDCG@k — 전달받은 CV와 모델 설정 기준으로 계산"""
...
def prepare_data(cfg):
    ...
    # brand_fame_log가 있으면 네이버 데이터랩 기반 브랜드 검색량 변수를 그대로 사용
    # 없을 때만 임시 대체값으로 데이터셋 내 브랜드 등장 빈도 로그값 사용
    if "brand_fame_log" not in df.columns:
        bc = df["brand"].value_counts()
        df["brand_fame_log"] = np.log1p(df["brand"].map(bc).fillna(0))
        print("  brand_fame_log 없음: 데이터셋 내 브랜드 등장 빈도 로그값으로 임시 대체")
...
for cat_name, cfg in CATEGORIES.items():
    ...
    print("  ── 리뷰수 제외 ──")
    for n, m in REG_MODELS.items():
        r = eval_reg(n, m, X_no_rev, y_score, "rank_score(리뷰수제외)")
        r["category"] = label; reg_results.append(r)
    ...
    imp_nr = shap_charts(sv_nr, X2_nr, NO_REVIEW,
                         ...
                         f"2단계 LightGBM {label} (top38, 리뷰수제외)", OUT_DIR)
    print(f"  상위 feature (리뷰수제외):\n{imp_nr.to_string()}")
